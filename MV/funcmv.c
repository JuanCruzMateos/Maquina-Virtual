...
void SMOV(int *a, int *b, memoria_t *mem) {
    ...
    origen = *b;
    destino = *a;
    while (mem->ram[origen] != 0) {
        mem->ram[destino] = mem->ram[origen];
        origen++;
        destino++;
    }
    mem->ram[destino] = mem->ram[origen];
...
void SCMP(int *a, int *b, memoria_t *mem) {
    ...
    origen = *b;
    destino = *a;
    while (iguales == 0 && !(mem->ram[origen] == 0 && mem->ram[destino] == 0)) {
        if (mem->ram[origen] - mem->ram[destino] > 0) {
            iguales = 1;
        } else if (mem->ram[origen]-mem->ram[destino]<0) {
            iguales = -1;
        }
        origen++;
        destino++;
